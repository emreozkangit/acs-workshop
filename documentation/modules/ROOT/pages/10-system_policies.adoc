...
+
. We will use ``Image Registry`` criteria at the ``Image Registry`` category as shown below.You can drag and drog and fill-up the box with ``docker.io``
+
...
+
. Create Policy repeat steps as shown below. We will create a new project for our tests.
+
...
+
. ``Image Name: docker.io/mysql:latest``  Click ``Create``
+
image::policies/console5.png[ACS Policies 22, 800]
+
. Let's review the error.
+
image::policies/console6.png[ACS Policies 23, 300]
+
